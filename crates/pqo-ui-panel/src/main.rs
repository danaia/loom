...
use pqo_windowing::{SnapManager, SnapUpdate, WindowFrame, WindowLayoutConfig, WindowPosition};
use serde::{Deserialize, Serialize};
use serde_json::{Value, json};
use tauri::{Manager, WebviewUrl, WebviewWindowBuilder, http};
use zeroize::Zeroizing;

const AGENT_KEYCHAIN_SERVICE: &str = "com.pqo.agent.openai";
...
fn open_agents_window(
    ...
    const LABEL: &str = "pqo-project-agents";
    ...
    let url = tauri::Url::parse("pqo-ui://localhost/agents.html")
...
fn has_agent_api_key(state: tauri::State<'_, PanelState>) -> Result<bool, String> {
    ...
    match entry.get_password() {
        ...
        Err(error) => Err(format!("could not read the Pqo Agents key: {error}")),
    }
}
...
fn connect_and_start_agent(
    ...
    let reply = request_agent_reply(
        api_key,
        "Confirm in one sentence that the active Pqo project is connected with read/write project tools and validated Metal hot reload enabled.",
        None,
        &state,
    )?;
...
fn load_api_key(state: &PanelState) -> Result<Zeroizing<String>, String> {
    ...
    entry
        .get_password()
        .map(Zeroizing::new)
        .map_err(|error| format!("could not read the Pqo Agents key from macOS Keychain: {error}"))
}
...
fn request_agent_reply(
    ...
    for _ in 0..AGENT_TOOL_MAX_ROUNDS {
        ...
        if calls.is_empty() {
            let text =
                response_output_text(&payload).unwrap_or_else(|| "Pqo agent connected.".to_owned());
    ...
    json!([
        {
            ...
            "description": "Read a UTF-8 source file inside the active Pqo project.",
            "parameters": {
                "type": "object",
                "properties": {
...
fn build_project_context(
    ...
    let project_name = files
        .iter()
        .find(|path| path.extension().and_then(|extension| extension.to_str()) == Some("pqo"))
        .and_then(|path| path.file_stem())
        .and_then(|name| name.to_str())
        .or_else(|| project_root.file_name().and_then(|name| name.to_str()))
        .unwrap_or("Pqo project")
        .to_owned();
...
fn is_ignored_directory(name: &str) -> bool {
    matches!(
        name,
        ".git" | ".pqo" | "node_modules" | "target" | "dist" | "build" | ".cache" | "agentDB"
    )
}
...
fn is_context_source(path: &Path) -> bool {
    ...
    if matches!(
        name,
        "package-lock.json" | "Cargo.lock" | "pqo-package.json"
    ) {
        return false;
    }
    matches!(
...
fn run() -> Result<(), String> {
    ...
    tauri::Builder::default()
        .manage(state)
        .register_uri_scheme_protocol("pqo-ui", move |_context, request| {
        ...
        .setup(move |app| {
            let url = tauri::Url::parse("pqo-ui://localhost/")
...
fn parse_arguments(arguments: impl Iterator<Item = String>) -> Result<PanelArguments, String> {
    ...
    Ok(PanelArguments {
        ...
        project_root,
        entry,
        title: title.unwrap_or_else(|| "Pqo Controls".to_owned()),
...
mod tests {
    ...
    fn extracts_responses_api_function_calls() {
        let payload = json!({
            "output": [{
                ...
                "call_id": "call_123",
                "name": "replace_in_project_file",
                "arguments": "{\"path\":\"baseline.pqo\",\"old_text\":\"cap=1\",\"new_text\":\"cap=2\"}"
            }]
        });
    ...
    fn project_context_includes_sources_and_excludes_secrets_and_generated_files() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("pqo-panel-context-{}-{unique}", std::process::id()));
        ...
        fs::write(root.join("baseline.pqo"), "module baseline {}").expect("pqo source");
        ...
        assert!(context.text.contains("baseline.pqo"));
        ...
        assert!(resolve_project_source(&root, "baseline.pqo", true).is_ok());
    ...
    fn loads_root_agents_instructions_for_the_embedded_agent() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("pqo-panel-agents-{}-{unique}", std::process::id()));
    ...
    fn agent_history_round_trips_without_entering_project_context() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("pqo-agent-history-{}-{unique}", std::process::id()));
        fs::create_dir_all(&root).expect("project directory");
        let database = AgentDatabase {
            version: AGENT_DB_VERSION,
            chats: vec![AgentChat {
                id: "chat-test".to_owned(),
                title: "Hello Pqo".to_owned(),
                messages: vec![AgentChatMessage {
                    role: "user".to_owned(),
                    text: "Hello".to_owned(),
                }],
    ...
    fn empty_agent_database_recovers_as_new_history() {
        ...
        let root = std::env::temp_dir().join(format!(
            "pqo-empty-agent-history-{}-{unique}",
            std::process::id()
        ));
