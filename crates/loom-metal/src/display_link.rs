...
impl DisplayLinkDriver {
    ...
    unsafe fn start_on_main_run_loop(layer: &MetalLayer) -> Result<Self, RuntimeDiagnostic> {
        ...
        unsafe {
            (*delegate).set_ivar("loomUpdateSender", sender_pointer as usize);
        }
...
fn display_link_delegate_class() -> &'static Class {
    ...
    CLASS.get_or_init(|| {
        let mut declaration = ClassDecl::new("LoomMetalDisplayLinkDelegate", class!(NSObject))
            .expect("display-link delegate class must be unique");
        declaration.add_ivar::<usize>("loomUpdateSender");
        unsafe {
...
extern "C" fn display_link_update(
    ...
    unsafe {
        let sender_pointer = *delegate.get_ivar::<usize>("loomUpdateSender");
