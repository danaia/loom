//! Agent-native Pqo source parsing and lowering.
//!
//! This crate intentionally starts with a narrow, deterministic grammar. It
//! lowers source directly into `pqo_core`'s canonical typed graph so the
...
/// Parse agent-native Pqo source and lower it into the canonical graph.
...
impl Parser {
    ...
    fn parse_module(&mut self) -> Result<ModuleAst, SourceDiagnostic> {
        ...
        self.expect_word("pqo")?;
        let version = self.expect_number()?;
        if version != "0.1" {
            return Err(self.error(
                "P0002",
                format!("unsupported Pqo version `{version}`; expected `0.1`"),
            ));
        }
...
fn generate_metal(
    ...
    Ok(packaged_metal_implementation(
        format!("pqo://generated/{module_name}/{}.metal", kernel.name),
        entry,
        source,
    ))
}
