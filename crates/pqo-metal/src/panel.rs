...
use pqo_windowing::{WindowFrame, WindowLayoutConfig, WindowPosition};
...
fn panel_executable() -> Result<PathBuf, RuntimeDiagnostic> {
    if let Some(path) = env::var_os("PQO_UI_PANEL_BIN") {
        let path = PathBuf::from(path);
        if path.is_file() {
            return Ok(path);
        }
        return Err(panel_error(format!(
            "PQO_UI_PANEL_BIN points to missing file `{}`",
            path.display()
        )));
    }
    let current = env::current_exe()
        .map_err(|error| panel_error(format!("could not locate Pqo executable: {error}")))?;
    ...
    Err(panel_error(format!(
        "project includes a UI, but neither `{}` nor its resolved-install sibling exists; \
         build or install the pqo-ui-panel helper",
        sibling.display()
    )))
}
