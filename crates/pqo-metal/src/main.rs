use pqo_core::{
    ...
    hello_crystal_builder, hello_organism_builder, hello_worm_builder,
};
...
fn run() -> Result<(), pqo_metal::RuntimeDiagnostic> {
    ...
    let (builder, benchmark) = match arguments.first().map(String::as_str) {
        ...
        Some("worm") => {
            let (_, benchmark) = parse_experiment_arguments(&arguments[1..], 1)?;
            (hello_worm_builder(), benchmark)
        }
        Some(other) => {
            return Err(pqo_metal::RuntimeDiagnostic {
                code: pqo_metal::RuntimeDiagnosticCode::UnsupportedGraph,
                message: format!(
                    "unknown experiment `{other}`; expected `particle`, `batch [COUNT]`, `population [CAPACITY]`, `field`, `organism [CAPACITY]`, `crystal [CUBIC_CELL_COUNT]`, or `worm`"
                ),
                semantic_path: None,
            });
        }
    ...
    let graph = builder
        .build()
        .map_err(|diagnostics| pqo_metal::RuntimeDiagnostic {
            code: pqo_metal::RuntimeDiagnosticCode::UnsupportedGraph,
    ...
    let validated = report
        .validated
        .ok_or_else(|| pqo_metal::RuntimeDiagnostic {
            code: pqo_metal::RuntimeDiagnosticCode::UnsupportedGraph,
...
fn parse_batch_arguments(
    arguments: &[String],
) -> Result<(u32, Option<BenchmarkConfig>), pqo_metal::RuntimeDiagnostic> {
        ...
        match arguments[index].as_str() {
            ...
            "--runner" => {
                let runner = match arguments.get(index + 1).map(String::as_str) {
                    Some("pqo") => BenchmarkRunner::PqoPlan,
                    Some("direct-metal") => BenchmarkRunner::DirectMetalEncoding,
                    _ => {
                        return Err(argument_error(
                            "--runner requires `pqo` or `direct-metal`".to_owned(),
                        ));
                    }
                };
                benchmark
            ...
            other => {
                return Err(argument_error(format!(
                    "unknown experiment option `{other}`; use `--bench [headless|rendered|presented]`, `--runner [pqo|direct-metal]`, `--pace HZ`, `--pace-lead-us N`, `--warmup N`, `--samples N`, `--warmup-seconds N`, or `--duration-seconds N`"
                )));
            }
        }
        index += 1;
...
fn parse_positive_option(
    ...
    index: usize,
    option: &str,
) -> Result<u32, pqo_metal::RuntimeDiagnostic> {
    arguments
        .get(index + 1)
...
fn parse_count(value: &str) -> Result<u32, pqo_metal::RuntimeDiagnostic> {
...
fn invalid_count(value: &str) -> pqo_metal::RuntimeDiagnostic {
    argument_error(format!(
        "invalid particle count `{value}`; use a positive integer or suffix such as `1k` or `1m`"
    ))
}

fn argument_error(message: String) -> pqo_metal::RuntimeDiagnostic {
    pqo_metal::RuntimeDiagnostic {
        code: pqo_metal::RuntimeDiagnosticCode::UnsupportedGraph,
        message,
        semantic_path: None,
    }
...
mod tests {
    ...
    use pqo_metal::{BenchmarkMode, BenchmarkRunner};
