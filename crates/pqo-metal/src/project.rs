...
pub(crate) const KEY_G: u32 = 9;
pub(crate) const KEY_SHIFT: u32 = 10;
...
impl ProjectExtension {
    pub(crate) fn load(path: &Path) -> Result<Self, RuntimeDiagnostic> {
        ...
        let abi_version = *unsafe { library.get::<AbiVersionFn>(b"pqo_project_abi_version_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_abi_version_v1", error))?;
        ...
        let title_fn = *unsafe { library.get::<TextFn>(b"pqo_project_title_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_title_v1", error))?;
        let help_fn = *unsafe { library.get::<TextFn>(b"pqo_project_help_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_help_v1", error))?;
        let create = *unsafe { library.get::<CreateFn>(b"pqo_project_create_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_create_v1", error))?;
        let destroy = *unsafe { library.get::<DestroyFn>(b"pqo_project_destroy_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_destroy_v1", error))?;
        let event = *unsafe { library.get::<EventFn>(b"pqo_project_event_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_event_v1", error))?;
        let frame = *unsafe { library.get::<FrameFn>(b"pqo_project_frame_v1\0") }
            .map_err(|error| missing_symbol(path, "pqo_project_frame_v1", error))?;
        let control = unsafe {
            library
                .get::<ControlFn>(b"pqo_project_control_v1\0")
                .ok()
                .map(|symbol| *symbol)
        };
    ...
    pub(crate) fn control(&mut self, name: &str, value: f32) -> Result<bool, RuntimeDiagnostic> {
        let Some(control) = self.control else {
            return Err(RuntimeDiagnostic::new(
                RuntimeDiagnosticCode::ProjectExtensionFailed,
                "the project panel requires `pqo_project_control_v1` in its extension",
            ));
        };
