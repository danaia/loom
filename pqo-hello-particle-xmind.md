# Pqo — Agent-Native Low-Level Physical Compute
...
- Compile Pqo programs into native ARM64 host code and Metal GPU kernels
...
- Pqo targets GPU-kernel level through Metal Shading Language and compiled Metal libraries
- Pqo targets native user-space ARM64 rather than a macOS kernel extension
...
- Prove the complete Pqo path with one particle
...
- Pqo must measure the CPU/GPU crossover rather than hard-code GPU use for every workload

## Canonical Pqo Representation
...
- `.pqo` canonical textual projection
- `.pqob` validated compiled bundle
...
- Custom SSA IR follows after Pqo semantics stabilize
...
- Hardware-neutral Pqo execution graph
...
The detailed gate definitions and acceptance criteria live in `pqo-plan.md`.
...
- Stabilize the smallest useful `.pqo` grammar
...
- Produce reproducible `.pqob` artifacts
...
- `pqo check hello-particle`
- `pqo build hello-particle --target apple-m4`
- `pqo run hello-particle`
