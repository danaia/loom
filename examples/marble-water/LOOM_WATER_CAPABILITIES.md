# Loom Water Simulation Capabilities

## Executive summary

Loom is a strong foundation for realistic real-time water simulation, but it is not yet a complete fluid-rendering engine.
...
That architecture is well suited to height-field water, surface diagnostics, foam fields, spray, bubbles, and other secondary effects. The largest gaps are in the render-resource model and the native `.loom` syntax—not in the core GPU simulation model.
...
## What Loom is made for

Loom v0 is a typed compute/render graph targeting Metal. It separates physical meaning from backend implementation:

```text
Loom streams and values
...
Complex fluid arithmetic does not need to fit inside the current native Loom kernel subset. It can remain in explicit external Metal kernels while Loom owns the state, units, access boundaries, bindings, dispatch domains, and schedule.

Relevant implementation boundary:

- `docs/gpu-authoring-handbook.md:30` describes Loom v0 as a typed compute/render graph with native and external Metal kernels.
- `crates/loom-metal/src/runtime.rs` performs the validated Metal lowering and execution.
- `examples/marble-water/marble-water.loom` already expresses a GPU-resident water and rigid-body coupling graph.
...
| Dynamic stream length in text `.loom` | Not yet supported | The parser currently accepts only integer `len` values. |
...
Loom validates that the kernel declares every stream it reaches and that conflicting passes are ordered.

### 3. Atomic effects and reductions

The Loom syntax and semantic model include atomic stream access. This is sufficient to represent:
...
This proves that Loom can execute and render large simple populations efficiently. It does not prove equivalent performance for a million-particle pressure solver, neighborhood search, or transparent fluid reconstruction. Fluid workloads must be benchmarked independently.
...
Reference: `crates/loom-metal/src/runtime.rs:1981`.
...
### 3. Dynamic populations are not expressible in text `.loom`
...
- `crates/loom-metal/src/runtime.rs:1849` lowers a dynamic count to indirect compute dispatch.

The text parser currently handles `len` as an integer only:

- `crates/loom-syntax/src/lib.rs:528`.

Consequences:

- builder-created graphs can use dynamic populations;
- the current distributable `marble-water.loom` cannot declare a count-backed pool;
...
The runtime owns the final drawable texture, but Loom compute and view bindings currently use stream buffers. There is no complete graph path for declaring and binding sampled, storage, depth, or offscreen textures.
...
Reference: `crates/loom-syntax/src/lib.rs:856`.
...
- `crates/loom-metal/src/runtime.rs:760`.
...
Loom can represent these as explicit external Metal passes with declared temporary streams and dependencies. The native kernel language and standard library do not yet expose them as reusable primitives.
...
## Recommended Loom capability gates
...
Loom is made for the simulation architecture required by realistic water:
...
Therefore, the recommended strategy is to extend Loom rather than replace it. Build the renderer and resource capabilities needed by the layered-water plan while preserving the existing validated stream-and-pass model. That keeps Marble Water aligned with Loom’s intended purpose and turns the example into a useful capability driver for the engine itself.
