# Pqo Implementation Roadmap

A gate-driven plan for building **Pqo**, an agent-native, low-level
physical-compute language.

Agents author and modify Pqo; deterministic compilers, validators, verifiers, and
runtimes decide what executes. In v0, Pqo is an early systems DSL with a low-level
...
The canonical system vision lives in `pqo-hello-particle-xmind.md`. The language charter and semantic model live in `docs/00-language-charter.md` and `docs/01-semantic-model.md`.

“Language first” means locking Pqo's constitution and primary composition patterns before building the runtime. It does not mean completing a general parser or polishing every syntax rule before anything executes.
...
- `pqo explain` can reveal inferred defaults, resolved bindings, effects, dependencies, layouts, and contract instrumentation.
...
- `examples/hello-particle/hello-particle.pqo` as the v0 language specimen.
- `crates/pqo-core` with typed graph nodes and a direct builder API.
- `crates/pqo-validator` with only the rules required by Hello Particle.
...
Semantic dependency edges are lowered to the appropriate Metal ordering mechanism. They do not encode backend-specific barriers in Pqo.
...
- **Heap allocation:** issued by Pqo host code and observed by configured allocator instrumentation.
- **Application copy:** CPU memory copy explicitly issued by Pqo.
- **Application blit:** Metal blit explicitly encoded by Pqo.
...
- `crates/pqo-runtime` consuming a validated Gate 0 graph.
...
## Gate 2 — Canonical `.pqo` Projection
...
- `.pqo` maps one-to-one onto the typed graph.
...
```text
pqo check    <source.pqo>
pqo format   <source.pqo>
pqo explain  <source.pqo> [qualified-name]
pqo project  <graph-or-bundle> -o <projection.pqo>
```

`pqo explain` is central to the agent experience. It shows resolved identity, types, units, bindings, effects, hazards, dependency paths, layout decisions, capabilities, and contract evidence.

### Deliverables and acceptance

- `crates/pqo-syntax` parser, source AST, formatter, and source mapping.
- `crates/pqo-cli` with `check`, `format`, `explain`, and `project`.
...
## Gate 3 — Ahead-of-Time `.pqob` Bundle

**Goal:** Compile validated `.pqo` and backend inputs into a reproducible `.pqob`, then reproduce Gate 1 from that artifact alone.

`.pqo` is the canonical textual projection. `.pqob` is the validated compiled bundle. They never share an extension.
...
```text
pqo build    <source.pqo> --target <target> -o <artifact.pqob>
pqo run      <artifact.pqob>
pqo bench    <artifact.pqob> [scenario] --output <report.json>
pqo inspect  <artifact.pqob> --next-snapshot
pqo project  <artifact.pqob> -o <projection.pqo>
pqo compare  <candidate-a> <candidate-b> --scenario <name>
```

### Acceptance

- Identical hermetic inputs and compiler identity produce byte-identical output.
- A clean runtime loads only the `.pqob`, recreates legal pipelines and dispatch, displays Hello Particle, and passes the same contracts.
...
**Goal:** Preserve Pqo semantics across execution targets.
...
- **One model, two encodings.** The graph is canonical; `.pqo` projects it as text and `.pqob` packages a compiled result.
...
5. Freeze the smallest useful `.pqo` grammar only after the end-to-end path passes.
