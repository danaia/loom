...
current parser, and `pqo check` for literal source authority.
...
The Pqo parameter order is the Metal buffer order. Typical mappings:
...
The runtime encodes Pqo f32x3 streams and values as 12 packed bytes. Use
...
Run `pqo check` and read `status` first:
...
4. Re-run `pqo check`.
5. Once valid, run `pqo explain` and inspect bindings and order.
6. Run and observe.
