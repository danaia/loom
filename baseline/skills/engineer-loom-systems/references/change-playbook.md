...
5. `loom check`, then `loom explain`.
...
6. Run `loom check`.
7. Inspect implementation class and order with `loom explain`.
...
Freeze the Loom signature before implementing Metal. Create an ABI table:

| Buffer | Loom parameter | Access/reach | Loom type/unit | MSL type |
| --- | --- | --- | --- | --- |
| 0 | first parameter | e.g. `rw` | e.g. `f32x3<m>` | `device packed_float3*` |

Then:

1. Match every `[[buffer(n)]]` to Loom parameter order.
2. Use `const device` for read-only streams and `constant &` for values.
3. Bind Loom f32x3 streams and values as `packed_float3`; the runtime encodes
...
6. Keep per-invocation access local unless the Loom slot declares `all`.
...
→ Loom const with same qualified name
→ pass value binding
...
edits run `loom build` and test the packaged UI.
...
- `crates/loom-syntax/src/lib.rs`;
- semantic nodes in `loom-core`;
...
- `loom-core` model, builder, canonical ordering, and typed IDs;
...
- **Graph owner:** `.loom` resources, signatures, bindings, and flow.
...
- simultaneous renames across UI/runtime/Loom without one owner;
...
| --- | --- |
| `.loom` constants/initializers | check, explain, run observation |
