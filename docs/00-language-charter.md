...
These rules define Loom. Changing one requires an explicit language decision record and a version boundary.
...
Persistent mutable state lives in named, typed streams. Loom has no ambient property bags, hidden heap objects, or undeclared global mutation.
...
Capacity, layout, storage, buffering, mutability, residency, and lifetime are separate concepts. Loom never overloads one word to mean another.
...
Loom has one typed semantic graph.

- The builder API constructs it directly.
- `.loom` is its canonical textual projection.
- `.loomb` is a validated, compiled artifact.

Parsing text is not required inside the compiler, optimizer, or runtime.

## Agent Experience

Loom is designed for agents as first-class authors.
...
Defaults are allowed only when they are deterministic, inspectable, target-independent in meaning, and printed by `loom explain`.
...
Experiments are named variants, not silent mutations of an accepted artifact. An agent can ask Loom to compare variants against the same scenarios and contracts.
...
A language feature belongs in Loom v0 only if Hello Particle requires it or it is necessary to preserve one of the constitutional rules.
